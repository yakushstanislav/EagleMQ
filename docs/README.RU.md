...
Использование пользователей является хорошим тоном при разработке многокомпонентных систем где каждый компонент выполняет определённую задачу на сервере. По умолчанию имя пользователя и пароль - eagle.
...
Флаги *flags* являются битовой последовательностью.
Очередь поддерживает три флага - QUEUE\_AUTODELETE, QUEUE\_FORCE\_PUSH и QUEUE\_ROUND\_ROBIN.
