...
EagleMQ это бесплатный, открытый, высокопроизводительный и легковесный менеджер очередей.
...
.disconnect
-----------------
Команда *.disconnect* отключает клиента от сервера.
...
Использование пользователей является хорошим тоном при разработке многокомпонентных систем где каждый компонент выполняет определённую задачу на сервере.
...
<table border="1">
    ...
    <tr>
        ...
        <td>13</td>
        <td>.queue_list</td>
        <td>Разрешения на получение списка очередей</td>
    </tr>
    <tr>
        <td>8</td>
        ...
        <td>14</td>
        <td>.queue_size</td>
        <td>Разрешения на получения размера очереди</td>
    </tr>
    <tr>
        <td>9</td>
...
Очереди могут использоваться в синхронном и асинхронном режиме. В синхронном режиме клиент полностью сам управляет очередь и работает по схеме запрос -> ответ.
...
Очередь поддерживает два флага - QUEUE\_AUTODELETE и QUEUE\_FORCE\_PUSH.
